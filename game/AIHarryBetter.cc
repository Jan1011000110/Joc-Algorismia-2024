...
//   }
  ...
  double heal_wizards(int wizard_id, vector<vector<S>> &M_good, vector<vector<S>> &M_bad, bool is_bad, bool act = false) {
  ...
  double attack_ghosts(int wizard_id, map<int, bool> &attacked_ghosts, vector<vector<S>> &M_ghosts, bool act = false) {
    if (round() > NUM_ROUNDS - 50) return 0.0; 
    Pos p = unit(wizard_id).pos;
    ...
    if (reachable and round() - unit(ghost_id).last_attack_received() > ROUNDS_NO_ATTACK_GHOST) {
      if (act) {
        try_move(wizard_id, false, true, true, M_ghosts);
...
//   }
  ...
  double attack_enemies(int wizard_id, vector<vector<S>> &M_enemies, bool act = false) {
    // MOVE WIZARDS TO ATTACK OTHER WIZARDS
    Pos p = unit(wizard_id).pos;
    auto [enemy_id, d, enemy_p, dir] = M_enemies[p.i][p.j];
    double worth_fight = calculate_fight(p, unit(enemy_id).player, d);
    if (worth_fight > THRESHOLD_FIGHT) {
      if (act) {
        try_move(wizard_id, false, true, true, M_enemies);
      }
      return worth_fight;
    }
    else {
  ...
  double collect_books(int wizard_id, vector<vector<S>> &M_books, bool act = false) {
  ...
  void move_wizards() {
    ...
    vector<vector<S>> M_books = BFS_pos(books_pos, true);
    vector<int> other_wizards = get_other_wizards();
    vector<vector<S>> M_enemies = BFS_id(other_wizards, false); 
    ...
    for (auto wizard_id : wizards(me())) {
      ...
      double z = Z*attack_enemies(wizard_id, M_enemies);
      double k = K*collect_books(wizard_id, M_books);
      double mx = max({x, y, z, k});
      order.emplace_back(mx, wizard_id);
    }
    sort(order.rbegin(), order.rend());
    for (auto [t, wizard_id] : order) {
      double x = X*heal_wizards(wizard_id, M_good, M_bad, unit(wizard_id).is_in_conversion_process());
      double y = Y*attack_ghosts(wizard_id, attacked_ghosts, M_ghosts);
      double z = Z*attack_enemies(wizard_id, M_enemies);
      double k = K*collect_books(wizard_id, M_books);
      if (t == x) {
        heal_wizards(wizard_id, M_good, M_bad, unit(wizard_id).is_in_conversion_process(), true);
      }
      else if (t == y) {
        attack_ghosts(wizard_id, attacked_ghosts, M_ghosts, true);
      }
      else if (t == z) {
        attack_enemies(wizard_id, M_enemies, true);
      }
      else {
        collect_books(wizard_id, M_books, true);
      }
    }
  }
